...
# Pętla przechodzi po wszystkich plikach w katalogu ccc.
# Dla każdego z nich sprawdzamy:
#   - czy to dowiązanie miękkie (-L)
#   - czy dowiązanie działa, czyli jego cel istnieje (-e)
# Jeśli tak, to wypisujemy dokąd prowadzi, jako ścieżkę względną do katalogu domowego (${HOME}).

for link in ccc/*; do
  if [ -L "$link" ] && [ -e "$link" ]; then
    realpath --relative-to="${HOME}" "$link"
  fi
done
